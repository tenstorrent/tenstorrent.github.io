...
You've powered it on. You've verified the chips. You've changed the default password and confirmed the Blackhole® cards are alive with the venerable <code>TT-SMI</code>. This guide continues your adventure. (If you haven't finished hardware setup yet, head to the {doc}`setup` guide. It walks through unboxing, first login, verifying the chips with tt-smi, and launching your first model in TT-Studio).
...
- **<span id="memory-castle">Memory Castle</span>** — a roguelike dungeon where 600 particles represent memory traffic through the DDR→L2→L1→Tensix hierarchy, driven by real chip telemetry
...
tt-studio
```
You will need a (free) Hugging Face access token to engage with models in TT-Studio. For more details on this, see <a href="setup.html#step-8-get-access-to-model-weights">step 8 of the QuietBox 2 setup guide</a>.
...
<div style="width:100%;max-width:560px;margin-inline:auto;">
<div style="position:relative;width:100%;aspect-ratio:16/9;background:#111;">
<iframe src="https://www.youtube-nocookie.com/embed/mrvqKeBtVvU?si=9X3qfKmQnSFeMzWh" title="YouTube video: TT-QuietBox 2" style="position:absolute;inset:0;width:100%;height:100%;border:0;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen loading="lazy"></iframe>
</div>
</div>

<div style="margin-top: 2.5rem;" aria-hidden="true"></div>
...
The basic loop is: write a prompt (or click "✨ Inspire me" to generate one), submit it, and browse your existing gallery while the generation runs. The processor stays busy. Newly finished clips appear in the gallery as they complete. The generation queue drains automatically so you don't have to babysit it.

**QuietBox 2 Video Generation Models:**
...
Each Tensix core is a programmable unit that combines matrix math engines with a RISC-V control processor. The 120 Tensix cores on each Blackhole chip are connected through a 2D mesh Network-on-Chip. There is no central dispatcher — computation moves through the mesh as packets, and cores coordinate directly. This is what the <a href="#memory-castle">Memory Castle</a> visualization in TT-Toplike is showing: the flow of read operations, write operations, cache hits, and misses through the DDR→L2→L1→Tensix hierarchy, rendered as particles navigating between layers of a dungeon.

The consequence of this architecture is that memory bandwidth is the primary resource, not clock speed. That is why the QuietBox 2 specs lead with 1024 GB/sec per card rather than GHz.

### TT-Metalium™: Open Source, Low Level Programming

**TT-Metalium** is Tenstorrent's low-level programming language for Tensix hardware. It exposes the mesh directly: you schedule work across cores, define data movement explicitly, and write kernels in C++ that run on the RISC-V processors embedded in each Tensix core. We called it "Metalium" because the goal is to let you get as close to the metal as possible.

Most users will never *need* to write TT-Metalium kernels directly. The model serving stack, the inference server, and the compiler toolchain handle that layer. But understanding the architecture is useful context for interpreting what you see in TT-Toplike's visualizations, and serves valuable background for anyone who wants to optimize model performance or contribute to the software stack.
...
One of the more memorable starting points for understanding Tensix execution is the **Particle Life Simulator** — a multi-particle physics simulation where thousands of particles with different attraction and repulsion rules evolve into complex emergent structures. It runs on Tensix cores in TT-Metalium and is visually immediate: you can see the simulation running on the chips and tune the parameters live.
...
- [Writing your first TT-Metalium kernel](https://docs.tenstorrent.com/tt-vscode-toolkit/lessons/explore-metalium/)
...
## Quick Reference for Tools on Your TT-QuietBox 2
